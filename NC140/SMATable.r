...
get_sma_row <- function(name, data, plus = FALSE){
  ...
  if (plus == FALSE){
    ...
    sma_row[2]  = output(sma(log10(data$length_cm)~log10(data$diameter_mm)))
    sma_row[3]  = output(sma(log10(data$path_length_plus)~log10(data$diameter_mm)))
    sma_row[4]  = output(sma(log10(data$tot_length_plus)~log10(data$diameter_mm)))
    sma_row[5]  = output(sma(log10(data$area_plus)~log10(data$volume_plus)))
    sma_row[6]  = "-"
    sma_row[7]  = output(sma(log10(data$tot_area_plus)~log10(data$tot_volume_plus)))
    sma_row[8]  = output(sma(log10(data$diameter_mm)~log10(data$volume_plus)))
    sma_row[9]  = "-"
    sma_row[10] = output(sma(log10(data$diameter_mm)~log10(data$tot_volume_plus)))
    sma_row[11] = output(sma(log10(data$length_cm)~log10(data$volume_plus)))
    sma_row[12] = output(sma(log10(data$path_length_plus)~log10(data$tot_volume_plus)))
    sma_row[13] = output(sma(log10(data$tot_length_plus)~log10(data$tot_volume_plus)))
    sma_row[14] = output(sma(log10(data$diameter_mm)~log10(data$area_plus)))
    sma_row[15] = "-"
    sma_row[16] = output(sma(log10(data$diameter_mm)~log10(data$tot_area_plus)))
    sma_row[17] = output(sma(log10(data$length_cm)~log10(data$area_plus)))
    sma_row[18] = output(sma(log10(data$path_length_plus)~log10(data$tot_area_plus)))
    sma_row[19] = output(sma(log10(data$tot_length_plus)~log10(data$tot_area_plus)))
    sma_row[20] = output(sma(log10(rm_zero$length_cm)~log10(rm_zero$stem_m + rm_zero$twig_m)))
    sma_row[21] = output(sma(log10(data$path_length_plus)~log10(data$tot_stem_m + data$tot_twig_m)))
    sma_row[22] = output(sma(log10(data$tot_length_plus)~log10(data$tot_stem_m + data$tot_twig_m)))
    sma_row[23] = output(sma(log10(rm_zero$stem_m + rm_zero$twig_m)~log10(rm_zero$diameter_mm)))
    sma_row[24] = output(sma(log10(data$tot_stem_m + data$tot_twig_m)~log10(data$diameter_mm)))
    sma_row[25] = output(sma(log10(rm_zero$stem_m + rm_zero$twig_m)~log10(rm_zero$volume_plus)))
    sma_row[26] = "-"
    sma_row[27] = output(sma(log10(data$tot_stem_m + data$tot_twig_m)~log10(data$tot_volume_plus)))
    sma_row[28] = output(sma(log10(data$diameter_ratio)~log10(data$diameter_mm/data$diameter_ratio)))
  }
  return(sma_row)
}
